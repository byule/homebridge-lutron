 ...
 * @private
 */
class Strip extends Common_1.Common {
    /**
     * Creates a light strip device.
     *
    ...
    constructor(homebridge, device, log) {
        ...
        this.onSetState = (value) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.device.status.state !== state || this.device.status.level !== level) {
                ...
                yield this.device.set({ state, level, luminance: this.device.status.luminance });
            }
        });
        /**
         ...
         * @param value The characteristic value from Homebrtidge.
         */
        this.onSetTemperature = (value) => __awaiter(this, void 0, void 0, function* () {
            const luminance = this.transformRange(value, [140, 500], [1800, 3000]);
            this.log.debug(`Strip Set Luminance: ${this.device.name} ${luminance}`);
            this.log.debug(`Strip Set Temperature: ${this.device.name} ${value}`);
            yield this.device.set({
     ...
     */
    transformRange(value, source, destination) {
